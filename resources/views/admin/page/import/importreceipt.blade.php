	...
	<div class="container receipt" >
		...
		<div class="row">
			<div class="col-md-6 col-md-offset-3 obj-center">
				<h1>Receipt import phone</h1>
				<h4>Import no.  {{$import->id}}</h4>
			</div>
		</div>
		<div class="row">
			<div class="col-md-12 receipt_info">
				<h4><strong>Employee name: </strong>{{$import->username}}</h4>
				<h4><strong>Total quantity: </strong>{{$import->total_quantity}}</h4>
				<h4><strong>Total price: </strong>{{$import->total_price}}</h4>
				<h4><strong>Created at: </strong>{{$import->created_at}}</h4>
				<table class="table table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="4" class="receipt_total" style="padding-right: 50px">{{number_format($import->total_price)}} đ</td>
						</tr>
					</tbody>
				</table>
			</div>
